using DataFrames, BioSequences, BioBackgroundModels, CSV, FASTX, Statistics
...
function add_position_sequences!(df::DataFrame, genome_path::String, genome_idx_path::String)
    ...
    Threads.@threads for entry in eachrow(df)
        entry.start > 0 ? (estart=entry.start) : (estart=1)
        entry.seq=BioBackgroundModels.fetch_sequence(entry.chr, scaffold_seq_record_dict ,estart, entry.end, '+')
    end
