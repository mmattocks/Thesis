...
ph2_constants=[X, popdist, voldist, mc_its, 2]
ph3_constants=[X, popdist, voldist, mc_its, 3]

ph2_priors=[LogNormal(log(20),log(2)),Beta(1,1),LogNormal(log(20),log(2)),Beta(1,1),Uniform(3,360),Normal(500.,200.)]
ph2_box=vcat(vcat(fill(base_scale,2)...),time_scale,volconst_scale)
ph2_box=hcat([4.,0.,4.,0.,3.,10.],ph2_box)
...
for (pth,prior,constants,box) in zip([e2ph, e3ph],[ph2_priors, ph3_priors],[ph2_constants,ph3_constants],[ph2_box, ph3_scale])
    if isfile(pth*"/ens")
        ...
        e=CMZ_Ensemble(pth,2000,obs, prior, constants, box, GMC_DEFAULTS)
    end
