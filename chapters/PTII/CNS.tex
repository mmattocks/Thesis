...
The \path{mc_noise} parameter can be estimated by calculating the standard deviation of repeated evaluations of a parameter vector with a reasonably high likelihood, given the data vectors. The suggested value is roughly appropriate for \num{1e6} Monte Carlo iterates, given the data vectors used in \autoref{ssec:a10periodisation}.

\section{\protect\path{Slice_Ensemble} usage}
A \path{Slice_Ensemble} consists of \path{Slice_Models}, whose likelihood is ass

\section{\protect\path{Thymidine_Ensemble} usage}
